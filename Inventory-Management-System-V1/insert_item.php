...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($count > 0) {
        ...
        try {
            $stmt = $conn->prepare("INSERT INTO inventory (item_name, purchase_date, delivery_date, property_number, serial_number, model_number, description, manufacturer_name, quantity_bought, designation, received_by, supplier_name, OR_number, amount, transfer_history, ordered_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$itemName, $purchaseDate, $deliveryDate, $propertyNumber, $serialNumber, $modelNumber, $description, $manufacturerName, $quantityBought, $designation, $receivedBy, $supplierName, $orNumber, $amount, $transferHistory, $orderedBy]);
