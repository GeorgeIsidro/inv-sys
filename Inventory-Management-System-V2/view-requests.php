...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['confirm'])) {
        ...
        $availableQuantity = $conn->query("SELECT quantity_remaining FROM inventory WHERE property_number = '{$propertyNumber}'")->fetchColumn();
        ...
        if ($availableQuantity >= $requestedQuantity) { // Check if available quantity is greater or equal to requested quantity
            ...
            $updateQtyStmt = $conn->prepare("UPDATE inventory SET quantity_remaining = quantity_remaining - ? WHERE property_number = ?");
		...
		$updateStmt = $conn->prepare("UPDATE inventory SET quantity_remaining = quantity_remaining + ? WHERE property_number = ?");
...
<body>
    ...
    <?php foreach ($itemRequests as $request) : ?>
        <tr>
            ...
            <td>
                <?php
                $itemQuantity = $conn->query("SELECT quantity_remaining FROM inventory WHERE property_number = '{$request['property_number']}'")->fetchColumn();
                if ($itemQuantity > 0) {
